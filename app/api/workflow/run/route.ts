...
export async function POST(req: NextRequest) {
  ...
  if (!rows.length) return NextResponse.json({ ok: false, message: 'Keine News gefunden', editorial });

  const videos = [];
  for (const row of rows) videos.push({ articleId: row.id, video: await runPipeline(row.id) });
  return NextResponse.json({ ok: true, count: videos.length, videos, editorial });
}
