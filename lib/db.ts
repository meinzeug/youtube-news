...
CREATE TABLE IF NOT EXISTS ai_usage(id INTEGER PRIMARY KEY AUTOINCREMENT, scenario TEXT NOT NULL, model TEXT NOT NULL, promptTokens INTEGER DEFAULT 0, completionTokens INTEGER DEFAULT 0, costUsd REAL DEFAULT 0, billingTier TEXT NOT NULL DEFAULT 'paid', createdAt TEXT DEFAULT CURRENT_TIMESTAMP);
CREATE TABLE IF NOT EXISTS editorial_campaigns(id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, topic TEXT NOT NULL, instructions TEXT NOT NULL DEFAULT '', cadenceMinutes INTEGER NOT NULL DEFAULT 60, status TEXT NOT NULL DEFAULT 'active', targetChannel TEXT NOT NULL DEFAULT 'YouTube', autoUpload INTEGER NOT NULL DEFAULT 0, nextRunAt TEXT, lastRunAt TEXT, lastResult TEXT, createdBy TEXT NOT NULL DEFAULT 'CEO', createdAt TEXT DEFAULT CURRENT_TIMESTAMP, updatedAt TEXT DEFAULT CURRENT_TIMESTAMP);
CREATE TABLE IF NOT EXISTS editorial_campaign_runs(id INTEGER PRIMARY KEY AUTOINCREMENT, campaignId INTEGER NOT NULL, articleId INTEGER, status TEXT NOT NULL DEFAULT 'queued', videoPath TEXT, youtubeUrl TEXT, log TEXT, startedAt TEXT, completedAt TEXT, createdAt TEXT DEFAULT CURRENT_TIMESTAMP);
CREATE TABLE IF NOT EXISTS crawl_runs(id INTEGER PRIMARY KEY AUTOINCREMENT, status TEXT NOT NULL DEFAULT 'running', activeSources INTEGER DEFAULT 0, dueSources INTEGER DEFAULT 0, succeededSources INTEGER DEFAULT 0, failedSources INTEGER DEFAULT 0, articlesImported INTEGER DEFAULT 0, log TEXT, startedAt TEXT DEFAULT CURRENT_TIMESTAMP, completedAt TEXT);`);
...
const aiUsageColumns = new Set((db.prepare('pragma table_info(ai_usage)').all() as { name: string }[]).map((column) => column.name));
if (!aiUsageColumns.has('billingTier')) db.exec("ALTER TABLE ai_usage ADD COLUMN billingTier TEXT NOT NULL DEFAULT 'paid'");
